...
```{r}
# loading the dataset - dataset includes information on 2025 NESCAC Women's Lacrosse players 
nescac_list <- read.csv("NESCAC_Mens_Lacrosse.csv")
nescac_list
```
...
# Add an gender column for each dataset
womens_data$Gender <- "Women"
mens_data$Gender <- "Men"

# Combine both into one dataset
combined_data <- rbind(womens_data[, c("Home.State", "School.State", "Gender")],
                       mens_data[, c("Home.State", "School.State", "Gender")])

# Compare homestates
home_table <- table(combined_data$Home.State, combined_data$Gender)
...
#library(vcd)
#assocstats(home_table)  # includes Cramér's V
